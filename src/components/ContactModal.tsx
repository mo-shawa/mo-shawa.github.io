...
import { dataButtonProps } from "@/pages"
type Props = {
  ...
  return (
    ...
    >
      <motion.div
        ...
        onClick={() => setContactModalOpen(false)}
        className="absolute inset-0 filter backdrop-blur-md"
      ></motion.div>
      <motion.div
        layoutId="contact"
      ...
      >
        ...
        <div className="flex flex-wrap justify-evenly gap-4 gap-y-10 ">
          ...
          <motion.button
            {...dataButtonProps}
            onClick={() => setContactModalOpen(false)}
            className="rounded-full bg-white px-12 py-3 font-medium text-black"
          >
            Close
          </motion.button>
        </div>
